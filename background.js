...
async function captureRequestingTab(senderTab, format = 'jpeg') {
  ...
  const options = format === 'png' ? { format: 'png' } : { format: 'jpeg', quality: 90 };
  // Mobile Chromium (Kiwi) throws "No active web contents to capture" transiently during
  // navigation, paint, or focus changes. These almost always succeed on a short-delay retry.
  const transient = /no active web contents|too many calls|MAX_CAPTURE/i;
  let lastError;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await chrome.tabs.captureVisibleTab(senderTab.windowId, options);
    } catch (error) {
      lastError = error;
      if (!transient.test(String(error?.message || error)) || attempt === 2) throw error;
