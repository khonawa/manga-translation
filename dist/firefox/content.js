(() => {
  ...
  async function initializePageTurnClearing() {
    ...
    autoTranslateEnabled = stored.autoTranslateEnabled === true;
    chrome.storage.onChanged.addListener(changes => {
      ...
      if (changes.autoTranslateEnabled) {
        autoTranslateEnabled = changes.autoTranslateEnabled.newValue === true;
        if (!autoTranslateEnabled) clearTimeout(autoTranslateTimer);
        syncPageTurnWatchers();
      }
    });
  ...
  function syncPageTurnWatchers() {
    const needed = (clearOverlaysOnPageTurn && hasVisibleOverlays()) || autoTranslateEnabled;
  ...
  function checkForPageTurn() {
    const wantsCheck = (clearOverlaysOnPageTurn && hasVisibleOverlays()) || autoTranslateEnabled;
    ...
    if (location.href !== observedPageUrl) {
      ...
      if (clearOverlaysOnPageTurn) clearAllOverlays({ sessionKey: previousSessionKey });
    ...
    else if (currentSignature !== translatedImageSignature) {
      if (clearOverlaysOnPageTurn) clearAllOverlays();
      scheduleAutoTranslate();
    }
  }

  function scheduleAutoTranslate() {
    if (!autoTranslateEnabled || activeRequestId) return;
    ...
    autoTranslateTimer = setTimeout(() => {
      ...
      const now = Date.now();
      if (now - lastAutoCaptureAt < 2000) return;
  ...
  async function initFloatPanel() {
    ...
    function syncAutoButton(btn) {
      btn.classList.toggle('manga-fp-active', autoTranslateEnabled);
      btn.title = `Auto Translate: ${autoTranslateEnabled ? 'ON' : 'OFF'}`;
      btn.setAttribute('aria-pressed', String(autoTranslateEnabled));
    }
  ...
  async function processFullPageTranslation({ silent = false } = {}) {
    ...
    try {
      ...
      if (silent && isCaptureFailure) {
        statusBanner.remove();
        if (activeStatus === statusBanner) activeStatus = null;
        return;
      }
